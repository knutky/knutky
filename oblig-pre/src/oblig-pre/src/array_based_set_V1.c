...
void set_add(set_t *set, void *member) {
    ...
    void **temp = realloc(set->members, sizeof(void*) * (set->length + 1));
    ...
    while (low <= high) {
        ...
        int cmp_result = set->cmpfunc(set->members[mid], member);
        if (cmp_result == 0) {
            // Member already in the set, do nothing
            return;
        } else if (cmp_result < 0) {
            low = mid + 1;
        } else {
    ...
    // Allocate memory for the new member and copy the value
    set->members[low] = malloc(sizeof(void));
    memcpy(set->members[low], member, sizeof(void));
...
void set_add(set_t *set, void *member) {
    ...
    // Perform insertion at the correct position
    void** new_members = realloc(set->members, sizeof(void*) * (set->length + 1));
    if (new_members == NULL) {
        // Handle reallocation failure (optional)
        return;
    }
    for (int i = set->length; i > low; i--) {
        set->members[i] = set->members[i - 1];
